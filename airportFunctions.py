...
class Airport: 
	...
	def __init__(self):
		...
		self.history_list = [] # autres avions (déjà attéri, décollé ou crashé)
		self.airlines = {} # dictionnaire de toutes les compagnies, avec leur ID
		self.tick = 0 # entier représentant les minutes écoulées (min: 0, max: 1439)
	...
	def create_plane (self, ID, company, passengers, fuel, consumption, time,statut):
		'''
		creation d'un avion
		'''
		newPlane = Plane(ID, company, passengers, fuel, consumption, time,statut)
		ID_letter = ID[:-4]
		if (company) not in self.airlines: #ajouter la compagnie au dictionnaire si elle n'y est pas déjà
	...
	def add_company(self):
		...
		while not ok:
			...
			company_ID = (str(input("Entrez l'ID de la compagnie (2 ou 3 lettres) :"))).upper()
		...
		if len(self.arrival_list) == 0:
			...
			for plane in self.arrival_list[1:]:
				if plane.ratio() < most_prior_plane.ratio():
					most_prior_plane = plane
					#si le nombre de tour restant est le même, on regarde le nombre de passagers
	...
	def show_list_info (self, plane_list):
		...
		if len(plane_list) == 0:
			...
			for plane in plane_list:
				print ('n°' + (str(count)).ljust(2,' ') + ': ', str(plane), end= ' ')
				self.showTime(plane.getTime())
				if plane.getStatut() != None:
	...
	def show_airline_info (self, company_ID):
		...
		plane_lists = [self.departure_list, self.arrival_list, self.history_list]
		count = 1
	...
	def update_status(self):
		...
		for plane in self.arrival_list:
			plane.update() #diminution du carburant en fonction de la consomation/tour
			
			if plane.isCrashed(): #si l'avion n'a plus de fuel, il se crashe
				print ("\nL'avion", plane.getID(), "n'a malheureusement pas pu attérire à temps. \nVous avez tué", plane.getPassengers(),'passagers./o\\')
	...
	def next_event (self):
		...
		if arrival_plane != None and arrival_plane.ratio() == 1:
		...
		elif departure_plane != None and self.convTupleToTick(departure_plane.getTime()) <= self.tick :
		...
		elif arrival_plane != None:
			most_prior_plane = arrival_plane
			most_prior_plane.setStatut('Landed')
			self.arrival_list.remove(most_prior_plane)
			print("\nL'avion", most_prior_plane.getID(), "a attéri.")
			
		if most_prior_plane != None:
	...
	def next_departure(self):
		...
		if departure_plane != None and self.convTupleToTick(departure_plane.getTime()) <= self.tick :
			most_prior_plane = departure_plane
			most_prior_plane.setStatut('Take Off') 
			self.departure_list.remove(most_prior_plane)
			print("\nL'avion", most_prior_plane.getID(), "a décollé.")
	...
	def ask_for_runway(self):
		...
		while not ok:
			try:
				...
				nbr_mixte_runway = int(input("Pistes pour l'atterisssage et le décollage:"))
				ok = True
			except:
	...
	def add_random_departure_plane (self):
		...
		if len(self.airlines)  == 0:
			...
			while not ok:
				try:	
					company = (str(input('Nom complet de la compgnie:'))).lower()
					ID_letter = (str(input('ID de la compagnie (2 ou 3 lettres):'))).upper()
					ok = True
				except:
		...
		statut = None
		newPlane= self.create_plane(ID, company, passengers, fuel, consumption, time, statut)
	...
	def showTime(self, tick):
		if type(tick) is int:	
			print (str(self.convTickToTuple(tick)[0])+"h"+str(self.convTickToTuple(tick)[1])+".")
		elif tick == None:
			print ("Arr. Plane", end='  ')
		else:
			print ((str(tick[0])).rjust(2,'0')+'h'+(str(tick[1])).rjust(2,'0'), end='  ')
	...
	def user_menu(self):
		...
		while answer != 'q':
			...
			if answer  == 'a':
				...
				newplane = self.create_plane(ID, company, passengers, fuel, consumption, time, statut)
				self.add_plane(newplane)
				print ("\nL'avion", newplane.getID(), "a été ajouté à la liste des avions au décollage") 
				...
				newplane = self.create_plane(ID, company, passengers, fuel, consumption, time, statut)
				self.add_plane (newplane)
				print ("\nL'avion", newplane.getID(), "a été ajouté à la liste des avions à l'atterissage") 
					...
					while not ok:
						try: 
							indice = (int(input('Entrez sa position dans la liste des avions au départ: '))-1)
				...
				self.show_airlines()
				print ("\nEntrez l'ID de la compagnie que vous voulez supprimer:", end=' ')

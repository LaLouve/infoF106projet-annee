...
from tkinter import *
import tkinter.font as tkFont #permet la mise en forme des textes des label, boutons, ect. par exemple changer la taille et la police d'écriture
...
class AirportGUI:
	...
	def __init__(self, root):
		...
		Label(column1, bd=7, bg='white', text="ARRIVALS", font=tkFont.Font(size=12)).pack()
		
		list_box_area = Frame(column1, bd=8, bg='white') #création de list_box et de la scrollbar associée
		scrollbar = Scrollbar(list_box_area, bg='#C0C0C0', activebackground='grey', troughcolor='#F5F5F5', orient=VERTICAL)
		self.list_box_arrivals = Listbox(list_box_area, height=25, width=25, bd=2, yscrollcommand=scrollbar.set)
		...
		Button(column1, text="Add", relief=GROOVE, bg='#C0C0C0', command=self.addArrivalButton).pack(side=LEFT)
		Button(column1, text="Add Random", relief=GROOVE, bg='#C0C0C0', command=self.addArrivalRandom).pack(side=LEFT)
		
		
		####### colonne 2 #########
		column2 = Frame(root, bd=3, bg='white') #création de la colonne
		column2.pack(side=LEFT)
		Label(column2, bd=7, bg='white', text="DEPARTURES", font=tkFont.Font(size=12)).pack()
		
		list_box_area = Frame(column2, bd=8, bg='white') #création de list_box et de la scrollbar associée
		scrollbar = Scrollbar(list_box_area, bg='#C0C0C0', activebackground='grey', troughcolor='#F5F5F5', orient=VERTICAL)
		self.list_box_departures = Listbox(list_box_area, height=25, width=25, bd=2, yscrollcommand=scrollbar.set)
		self.list_box_departures.bind("<<ListboxSelect>>", self.checkPlaneDelete)
		self.list_box_departures.bind("<Double-Button-1>", self.infoDeparturePlane)
		...
		Button(column2, text="Add", relief=GROOVE, bg='#C0C0C0', command=self.addDepartureButton).pack(side=LEFT)
		Button(column2, text="Add Random", relief=GROOVE, bg='#C0C0C0', command=self.addDepartureRandom).pack(side=LEFT)
		self.delPlaneButton = Button(column2, text="Del", relief=GROOVE, bg='#C0C0C0', state=DISABLED, command=self.deletePlaneButton)
		...
		column3_partTOP= Frame(column3, bg='white') #partie haute de la troisième colonne, contient l'heure et le bouton step
		column3_partTOP.pack(side=TOP)
		self.general_time = self.time(airport.tick)
		self.clock = Label(column3_partTOP, bd=6, bg='white', text=self.general_time, font=tkFont.Font(size=16))
		self.clock.pack(side=TOP)
		
		setTime= Frame(column3_partTOP, bd=4, bg='white') #entrée du temps et bouton step
		setTime.pack()
		self.nbr_min = Entry(setTime, bg='white', justify=CENTER, relief=SUNKEN, bd=2, width=6)
		self.nbr_min.pack(side=LEFT)
		Label(setTime, bg='white', text='min').pack(side=RIGHT)
		Button(column3_partTOP, text='Step', relief=GROOVE, width=6, bg='#C0C0C0', command= lambda:self.stepButton(self.nbr_min.get())).pack(side=BOTTOM)
		
		column3_partCENTER= Frame(column3, height=220, bg='white').pack()
		
		column3_partBOTTOM= Frame(column3, bd=20, bg='white') #partie basse de la troisième colonne, contient les boutons "history", "companies" et "help"
		column3_partBOTTOM.pack(side=BOTTOM)
		Button(column3_partBOTTOM, text='Help', relief=GROOVE, width=12, bg='#C0C0C0', command=self.helpButton).pack(side=BOTTOM)
		Button(column3_partBOTTOM, text='History', relief=GROOVE, width=12, bg='#C0C0C0', command=self.historyButton).pack(side=BOTTOM)
		Button(column3_partBOTTOM, text='Companies', relief=GROOVE, width=12, bg='#C0C0C0', command=self.companiesButton).pack(side=BOTTOM)
		Button(column3_partBOTTOM, text='Runway', relief=GROOVE, width=12, bg='#C0C0C0', command=self.runwayButton).pack(side=BOTTOM)
	...
	def addButton(self, title, list_plane, list_box):
		...
		add_frame= Frame(self.add_window, bd=15, bg='white') #frame principale
		add_frame.pack()
		
		Label(add_frame, bd=4, bg='white', text='ID').grid(row=0, column=0)
		Id_frame= Frame(add_frame, bd=4, bg='white') #frame secondaire, contient l'entrée de l'ID
		Id_frame.grid(row=0, column=1)
		ID_letter = Entry(Id_frame, bd=2, bg='white', textvariable=str, justify=CENTER, relief=SUNKEN, width=5) 
		ID_letter.grid(row=0, column=0)
		ID_number = Entry(Id_frame, bd=2, bg='white', textvariable=int, justify=CENTER, relief=SUNKEN, width=7)
		ID_number.grid(row=0, column=1)
		

		Label(add_frame, bd=4, bg='white', text='Company').grid(row=1, column=0)
		company = Entry(add_frame, bd=2, bg='white', textvariable=str, justify=CENTER, relief=SUNKEN, width=13)
		company.grid(row=1, column=1)
		
		Label(add_frame, bd=4, bg='white', text='Passengers').grid(row=2, column=0)
		passengers = Entry(add_frame, bd=2, bg='white', textvariable=int, justify=CENTER, relief=SUNKEN, width=13)
		passengers.grid(row=2, column=1)
		
		Label(add_frame, bd=4, bg='white', text='Fuel').grid(row=3, column=0)
		fuel = Entry(add_frame, bd=2, bg='white', textvariable=int, justify=CENTER, relief=SUNKEN, width=13)
		fuel.grid(row=3, column=1)
		
		Label(add_frame, bd=4, bg='white', text='Consumption').grid(row=4, column=0)
		consumption = Entry(add_frame, bd=2, bg='white', textvariable=int, justify=CENTER, relief=SUNKEN, width=13)
		consumption.grid(row=4, column=1)
		
		if list_plane == airport.departure_list:
			Label(add_frame, bd=4, bg='white', text='Time').grid(row=5, column=0)
			time_frame= Frame(add_frame, bd=4, bg='white') #frame secondaire, contient l'entrée de l'heure
			time_frame.grid(row=5, column=1)
			heure = Entry(time_frame, bd=2, bg='white', textvariable=int, justify=CENTER, relief=SUNKEN, width=6)
			heure.grid(row=0, column=0)
			minute = Entry(time_frame, bd=2, bg='white', textvariable=int, justify=CENTER, relief=SUNKEN, width=6)
		...
		Button(add_frame, text="Add", relief=GROOVE, bg='#C0C0C0', \
		command=lambda:self.get_plane(ID_letter, ID_number, company,\
		passengers, fuel, consumption, heure, minute, statut, list_box)).grid(row=7, column=1)

	def get_plane(self, ID_letter, ID_number, company, passengers, fuel, consumption, heure, minute, statut, list_box):
		'''
				...
				if heure != None and minute != None:
					...
					if nbr_heure.isdigit() and nbr_minute.isdigit():
						if  int(nbr_heure) >= 0 and int(nbr_heure) <= 23 and int(nbr_minute) >= 0 and int(nbr_minute) <= 59:
							time = (str(nbr_heure).rjust(2, '0'), str(nbr_minute).rjust(2, '0'))
							plane = airport.create_plane(ID, name_company, nbr_passengers, nbr_fuel, nbr_consumption, time, statut)
	...
	def addDepartureButton(self):
		...
		'''
		plane = self.addButton("Add Departure Plane", airport.departure_list, self.list_box_departures)
		
	def addArrivalButton(self):
		'''
		Ajoute un avion dans la liste des arrivées
		'''
		plane = self.addButton("Add Arrival Plane", airport.arrival_list, self.list_box_arrivals)		
	...
	def helpButton(self):
		...
		Label(text_help, bd=6, bg='white', text="Bienvenue dans le simulateur de gestion d'aéroport", font=tkFont.Font(size=9)).pack(side=TOP)	
		...
		message = Message(text_help, text=text, bd=5, bg='white', anchor=CENTER)
		message.pack()
	...
	def historyButton(self):
		...
		column = Frame(history_window, bd=3, bg='white') #création de la colonne
		column.pack(side=LEFT)
		Label(column, bd=7, bg='white', text='History', font=tkFont.Font(size=12)).pack()
		
		list_box_area = Frame(column, bd=8, bg='white') #création de list_box et de la scrollbar associée
		scrollbar = Scrollbar(list_box_area, bg='#C0C0C0', activebackground='grey', troughcolor='#F5F5F5', orient=VERTICAL)
		self.list_box_history = Listbox(list_box_area, height=10, width=25, bd=2, yscrollcommand=scrollbar.set)
	...
	def companiesButton(self):
		...
		column = Frame(airlines_window, bd=3, bg='white') #création de la colonne
		column.pack(side=LEFT)
		self.column_company = column
		Label(column, bd=7, bg='white', text='Companies', font=tkFont.Font(size=12)).pack()
		
		list_box_area = Frame(column, bd=8, bg='white') #création de list_box et de la scrollbar associée
		scrollbar = Scrollbar(list_box_area, bg='#C0C0C0', activebackground='grey', troughcolor='#F5F5F5', orient=VERTICAL)
		self.list_box_company = Listbox(list_box_area, height=10, width=25, bd=2, yscrollcommand=scrollbar.set)
		self.list_box_company.bind("<Double-Button-1>", self.showInfoCompany)
		self.list_box_company.bind("<<ListboxSelect>>", self.checkCompanyDelete)
		self.list_airlines=[]
		for company in airport.airlines:
			text = (str(company).ljust(10,' ')+ str(airport.airlines[company]))
		...
		self.delCompanyButton = Button(column, text='Delete', relief=GROOVE, width=12, bg='#C0C0C0', state=DISABLED, command=self.deleteCompanyButton)
		self.delCompanyButton.pack(side=BOTTOM)
		Button(column, text='Add', relief=GROOVE, width=12, bg='#C0C0C0', command=self.addCompany).pack(side=BOTTOM)
	...
	def addCompany(self):
		...
		label_frame= Frame(self.add_airlines, bd=3, bg='white') #frame du label
		label_frame.grid(row=0, column=1)
		Label(label_frame, bd=6, bg='white', text='Add Airlines', font=tkFont.Font(size=10)).grid(row=0, column=1)

		entry_frame= Frame(self.add_airlines, bd=5, bg='white') #frame des entry
		entry_frame.grid(row=1, column=1)
		Label(entry_frame, bd=4, bg='white', text='Full Name').grid(row=0, column=0)
		company = Entry(entry_frame, bd=2, bg='white', textvariable=str, justify=CENTER, relief=SUNKEN, width=12)
		company.grid(row=0, column=1)

		Label(entry_frame, bd=4, bg='white', text='ID').grid(row=1, column=0)
		company_ID = Entry(entry_frame, bd=2, bg='white', textvariable=str, justify=CENTER, relief=SUNKEN, width=12)
		company_ID.grid(row=1, column=1)
	
		Button(self.add_airlines, text='Add', relief=GROOVE, width=10, bg='#C0C0C0', command= lambda:self.addInAirlines((company.get()).lower(), (company_ID.get()).upper())).grid(row=3, column=1)
	...
	def infoPlane(self, plane_list, plane):
		...
		principal = Frame(self.info_plane_window, bd=3, bg='white') # création de la frame principale
		principal.grid(row=0, column=0)
		
		frame_ID = Frame(principal, bd=5, bg='white') # frame secondaire, contient l'id
		frame_ID.grid(row=0, column=0)
		label_ID = Label(frame_ID, bd=3, bg='white', text='ID', font=tkFont.Font(size=9)).grid(row=0, column=0)
		info_ID = Label(frame_ID, bd=3, bg='white', text=plane.getID()).grid(row=1, column=0)
		
		frame_company = Frame(principal, bd=5, bg='white') # frame secondaire, contient la compagnie
		frame_company.grid(row=0, column=1)
		label_company = Label(frame_company, bd=3, bg='white', text='Company', font=tkFont.Font(size=9)).grid(row=0, column=0)
		info_compnay = Label(frame_company, bd=3, bg='white', text=plane.getCompany()).grid(row=1, column=0)
		
		frame_passengers = Frame(principal, bd=5, bg='white') # frame secondaire, contient le nombre de passagers
		frame_passengers.grid(row=0, column=2)
		label_passengers = Label(frame_passengers, bd=3, bg='white', text='Passengers', font=tkFont.Font(size=9)).grid(row=0, column=0)
		info_passengers = Label(frame_passengers, bd=3, bg='white', text=plane.getPassengers()).grid(row=1, column=0)
		
		frame_fuel = Frame(principal, bd=5, bg='white') # frame secondaire, contient le fuel
		frame_fuel.grid(row=0, column=3)
		label_fuel = Label(frame_fuel, bd=3, bg='white', text='Fuel', font=tkFont.Font(size=9)).grid(row=0, column=0)
		info_fuel = Label(frame_fuel, bd=3, bg='white', text=plane.getFuel()).grid(row=1, column=0)
		
		frame_consumption = Frame(principal, bd=5, bg='white') # frame secondaire, contient la consommation
		frame_consumption.grid(row=0, column=4)
		label_consumption = Label(frame_consumption, bd=3, bg='white', text='Consumption', font=tkFont.Font(size=10)).grid(row=0, column=0)
		info_consumption = Label(frame_consumption, bd=3, bg='white', text=plane.getConsumption()).grid(row=1, column=0)

		if plane_list == airport.departure_list:
			frame_time = Frame(principal, bd=5, bg='white') # frame secondaire, contient l'heure
			frame_time.grid(row=0, column=5)
			label_time = Label(frame_time, bd=3, bg='white', text='time', font=tkFont.Font(size=9)).grid(row=0, column=0)
			time = plane.getTime()
			text= (str(time[0])+'h'+str(time[1]))
			info_time = Label(frame_time, bd=3, bg='white', text=text).grid(row=1, column=0)

		if plane_list == airport.history_list:
			frame_statut = Frame(principal, bd=5, bg='white') # frame secondaire, contient le statut
			frame_statut.grid(row=0, column=5)
			label_statut = Label(frame_statut, bd=3, bg='white', text='statut', font=tkFont.Font(size=9)).grid(row=0, column=0)
			info_statut = Label(frame_statut, bd=3, bg='white', text=plane.getStatut()).grid(row=1, column=0)
		
		frame_button = Frame(principal, bd=5, bg='white')
		frame_button.grid(row=1, column=2)
		button_OK = Button(frame_button, text='OK', relief=GROOVE, width=6, bg='#C0C0C0', command=lambda:self.planeButtonOK()).grid(row=0, column=0)
	...
	def infoCompanyPlane(self, company_ID):
		...
		principal = LabelFrame(self.info_company_window, bd=5, bg='white',relief=RIDGE, text=text_label, font=tkFont.Font(size=10))
		principal.pack()
		
		list_box_area = Frame(principal, bd=8, bg='white') #création de list_box et de la scrollbar associée
		scrollbar = Scrollbar(list_box_area, bg='#C0C0C0', activebackground='grey', troughcolor='#F5F5F5', orient=VERTICAL)
		self.list_box_company_plane = Listbox(list_box_area, height=10, width=25, bd=2, yscrollcommand=scrollbar.set)
		self.list_box_company_plane.bind("<Double-Button-1>", self.infoPlaneCompany)
		plane_lists = [airport.departure_list, airport.arrival_list, airport.history_list]
		self.list_of_plane = []
		...
		button_OK = Button(principal, text='OK', relief=GROOVE, width=6, bg='#C0C0C0', command=lambda:self.companyButtonOK()).pack(side=BOTTOM)
		...
		if ok:
			for i in range (int(nbr_min)):
				...
				if plane != None:
					...
					if plane.isCrashed():
						text = "L'avion {} n'a malheureusement pas pu attérire à temps. \nVous avez tué {} passagers. /o\\".format(plane.getID(), plane.getPassengers())
	...
	def runwayButton(self):
		...
		label_principal= LabelFrame(self.runway_window, bd=4, relief=RIDGE, bg='white', text='Runways', font=tkFont.Font(size=10))
		label_principal.pack()
		
		departure_frame= Frame(label_principal, bd=6, bg='white')
		departure_frame.pack(side=TOP)
		text_departure_label = "Departure runways: {}".format(airport.departure_runway)
		departure_label= Label(departure_frame, bd=3, bg='white', text=text_departure_label).pack(side=RIGHT)
		
		arrival_frame= Frame(label_principal, bd=6, bg='white')
		arrival_frame.pack(side=TOP)
		text_arrival_label = "Arrival runways: {}".format(airport.arrival_runway)
		arrival_label= Label(arrival_frame, bd=3, bg='white', text=text_arrival_label).pack(side=RIGHT)		
		...
		mixte_label= Label(mixte_frame, bd=3, bg='white', text=text_mixte_label).pack(side=RIGHT)
		
		button_frame= Frame(label_principal, bd=5, bg='white')
		button_frame.pack(side=BOTTOM)
		add_button= Button(button_frame, text='Add', relief=GROOVE, width=6, bg='#C0C0C0', command=self.addRunwayButton)
		add_button.pack(side=LEFT)
		del_button =Button(button_frame, text='Delete', relief=GROOVE, width=6, bg='#C0C0C0', command=self.delRunwayButton)
	...
	def addRunwayButton(self):
		...
		label_principal= LabelFrame(self.add_runway_window, bd=4, relief=RIDGE, bg='white', text='Add Runways', font=tkFont.Font(size=10))
		label_principal.pack()
